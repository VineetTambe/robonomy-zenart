...
class Graph:
	...
	def saveAsXYDotFile(self, f):
		for node in self.nodes:
			f.write(node.toXYFormat())
	...
	def saveAsXYcodeFile(self, f):
		### First follow all paths that have a start/end node (i.e. are not cycles)
		# The next chosen starting node is the closest to the current position

		def pathToFile(i, insidePath):
			if insidePath:
		...
		if len(possibleStartingNodes) != 0:
			...
			while 1:
				possibleStartingNodes.remove(node)
				pathEndNode = pathToFile(node, False)
		...
		def cyclepathToFile(i, insidePath):
			if insidePath:
				# f.write(f"{self[i].y} {-self[i].x}
				f.write(f"{self[i].y},{-self[i].x}\n")
...
def main():
	# class Args: pass
	# parseArgs(Args)
	# image = Image.open("../data/images/hut.png")
	image = imageio.imread("../data/images/stary_night.png")
	...
	# 	edges = image
	edges = convertToBinaryEdges(edges, 100)

	# plt.imshow(edges,aspect="auto")
	# plt.show()


	converter = EdgesToGcode(edges = edges)
	converter.buildGraph()
	# f = open("hut.dot", "w")
	
	# converter.graph.saveAsXYDotFile(f)
	# if Args.dot_output is not None:
	f2 = open("../data/XYfiles/stary_night.dot", "w")

	converter.graph.saveAsXYcodeFile(f2)
